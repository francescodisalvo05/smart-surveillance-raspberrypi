    ...
    def set_model(self): # units = num classes
        ...
        if self.model_name == "DS-CNN":
            ...
            model = keras.Sequential([
                ...
                keras.layers.Conv2D(filters=int(self.alpha*64), kernel_size=(3,3), padding="same", activation="relu"),
                keras.layers.MaxPool2D(pool_size=(2,2), strides=(2,2), padding="same"),  # shapes problem here

                keras.layers.Conv2D(filters=int(self.alpha*128), kernel_size=(3,3), padding="same", activation="relu"),
                keras.layers.Conv2D(filters=int(self.alpha*128), kernel_size=(3,3), padding="same", activation="relu"),
                keras.layers.MaxPool2D(pool_size=(2,2), strides=(2,2), padding="same"),

                keras.layers.Conv2D(filters=int(self.alpha*256), kernel_size=(3,3), padding="same", activation="relu"),
                keras.layers.Conv2D(filters=int(self.alpha*256), kernel_size=(3,3), padding="same", activation="relu"),
                keras.layers.Conv2D(filters=int(self.alpha*256), kernel_size=(3,3), padding="same", activation="relu"),
                keras.layers.MaxPool2D(pool_size=(2,2), strides=(2,2), padding="same"),

                keras.layers.Conv2D(filters=int(self.alpha*512), kernel_size=(3,3), padding="same", activation="relu"),
                keras.layers.Conv2D(filters=int(self.alpha*512), kernel_size=(3,3), padding="same", activation="relu"),
                keras.layers.Conv2D(filters=int(self.alpha*512), kernel_size=(3,3), padding="same", activation="relu"),
                keras.layers.MaxPool2D(pool_size=(2,2), strides=(2,2), padding="same"),
            ...
            model = keras.Sequential([
                keras.layers.Conv2D(filters=int(self.alpha*64), kernel_size=(3,3), strides=(1,1), padding="same", activation="relu", input_shape=self.input_shape, name='conv1'),
                keras.layers.MaxPool2D(pool_size=(2,2), strides=(2,2), padding="same", name='pool1'),
                
                keras.layers.Conv2D(filters=int(self.alpha*128), kernel_size=(3,3), strides=(1,1), padding="same", activation="relu", name='conv2'),
                keras.layers.MaxPool2D(pool_size=(2,2), strides=(2,2), padding="same", name='pool2'),
                
                keras.layers.Conv2D(filters=int(self.alpha*256), kernel_size=(3,3), strides=(1,1), padding="same", activation="relu", name='conv3/conv3_1'),
                keras.layers.Conv2D(filters=int(self.alpha*256), kernel_size=(3,3), strides=(1,1), padding="same", activation="relu", name='conv3/conv3_2'),
                keras.layers.MaxPool2D(pool_size=(2,2), strides=(2,2), padding="same", name='pool3'),
                
                keras.layers.Conv2D(filters=int(self.alpha*512), kernel_size=(3,3), strides=(1,1), padding="same", activation="relu", name='conv4/conv4_1'),
                keras.layers.Conv2D(filters=int(self.alpha*512), kernel_size=(3,3), strides=(1,1), padding="same", activation="relu", name='conv4/conv4_2'),
